...
import (
	...
	beaconConsensus "github.com/dogeum-network/go-dogeum/consensus/beacon"
...
func setupBlocks(t *testing.T, ethservice *eth.Dogeum, n int, parent *types.Header, callback func(parent *types.Header), withdrawals [][]*types.Withdrawal) []*types.Header {
	api := NewConsensusAPI(ethservice)
...
func setupBodies(t *testing.T) (*node.Node, *eth.Dogeum, []*types.Block) {
	genesis, blocks := generateMergeChain(10, true)
	n, ethservice := startEthService(t, genesis, blocks)
